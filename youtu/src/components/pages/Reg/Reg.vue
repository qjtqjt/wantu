<template>
	<div class="login" >
		...
		<div class="main">
			...
			<div class="phone02">
				<input type="text" class="password" v-model="num" placeholder="验证码"/>
				<input type="text" class="code" v-model="btntxt" @click="code"></input>
			</div>
			<div class="text">
...
//			    	alert("邮箱格式不正确");
			...
			code(){
		    	...
		    	.then((res)=>{
					console.log(res);
					if(res.code==0){
						var yan_btn=document.getElementsByClassName("code")[0];
						yan_btn.disabled="disabled";
						this.codetimer(yan_btn);
			...
			//点击注册按钮
			regbtn(){				
				this.key=this.email&&this.pass&&this.num;
				if(!this.key){
					alert("请先填好信息");
					return ""
				}
